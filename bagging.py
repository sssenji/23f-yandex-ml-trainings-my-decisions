...
class SimplifiedBaggingRegressor:
        ...
        for i in range(len(self.data)):
          ...
          for bag in range(self.num_bags):
            if i not in self.indices_list[bag]:
              predictions.append(float(self.models_list[bag].predict(sample)))
    ...
    def _get_averaged_oob_predictions(self):
        ...
        for i in range(len(self.data)):
            ...
            if len(predictions) == 0:
                ...
                self.oob_predictions[i] = sum(predictions) / len(predictions)
    ...
    def OOB_score(self):
        ...
        # Your Code Here
        mask = self.oob_predictions is None
        return np.nanmean((self.oob_predictions[~mask] - self.target[~mask]) ** 2)